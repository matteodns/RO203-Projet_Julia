...
function readInputFile(inputFile::String)
    ...
    N = Int64(length(data))
    initMat = Matrix{Int64}(undef, N, N)
...
function displayGrid(initMat::Matrix{Int64})
...
function displaySolution(initMat::Matrix{Int64})
    
    isSolved = cplexSolve(initMat)[1]
    Y = cplexSolve(initMat)[3]
    ...
    if isSolved
        ...
        println("La grille n'admet pas de solution")
    end
end

function displaySolHeuristique(initMat::Matrix{Int64})
    
    isSolved = heuristicSolve(initMat)[1]
    Y = heuristicSolve(initMat)[3]
    ...
    for file in readdir(resultFolder)
        ...
        if isdir(path)
            ...
            for resultFile in filter(x->occursin(".txt", x), readdir(path))
                ...
                solveTime = Float64(0)
                isOptimalString = String("true")

                for line in lines
                    
                    if startswith(line, "solveTime = ")
                        solveTime = parse(Float64, SubString(line, 13:length(line)))
                    end
                    if startswith(line, "isOptimal = ")
                        isOptimalString = SubString(line, 13:length(line))
                    end
                end

                isOptimal = isOptimalString == true
        ...
        for method in folderName
            ...
            if isfile(path)
                ...
                for line in lines
                    ...
                    if startswith(line, "isOptimal = ")
                        isOptimal = SubString(line, 13:length(line))
                    end
                end

                print(solveTime, " ", isOptimal, "\n")

                println(fout, " & ", round(solveTime, digits=5), " & ", isOptimal)
